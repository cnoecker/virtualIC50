...
bemkl_multitask_supervised_regression_variational_train <- function(Km, y, parameters) {    
  ...
  for (iter in 1:parameters$iteration) {
    ...
    # update epsilon
    for (o in 1:T) {
      epsilon$scale[o] = 1 / (1 / parameters$beta_epsilon + 0.5 * as.double(crossprod(y[[o]], y[[o]]) - 2 * crossprod(y[[o]], crossprod(rbind(matrix(1, 1, N[o]), G[[o]]$mean), be$mean[c(o, (T + 1):(T + P))])) + N[o] * btimesbT.mean[o, o] + sum(diag(GtimesGT.mean[[o]] %*% etimeseT.mean)) + 2 * crossprod(rowSums(G[[o]]$mean), etimesb.mean[,o])))
    }
    print(epsilon$scale)
    # update b and e
